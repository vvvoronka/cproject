/**
 * @file AbcUtils.cpp
 * @brief Реализация утилит для работы с ABC (Berkeley-abc)
 * @details Содержит реализацию функций для взаимодействия с ABC через командную строку
 */
...
/// @brief Ключевое слово для идентификации команд ABC
static std::string d_utilWord = "abc ";
/// @brief Длина служебной информации о команде
static int d_utilLen = 8;

/// @brief Список команд, которые могут выводить результат
static std::vector<std::string> d_allowedOutput = {"read", "print", "map"};
/// @brief Список слов, указывающих на наличие ошибки в выводе
static std::vector<std::string> d_incorrectWords = {"Error", "failed",
                                                    "Cannot"};

/**
 * @brief Стандартный исполнитель команд ABC
 ...
 * @details Функция выполняет команду через popen и обрабатывает вывод,
 * анализируя его на наличие ошибок и результатов выполнения
 */
